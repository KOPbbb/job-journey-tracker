...
桌面版把投递记录保存在应用专用 LocalStorage，把上传的简历文件保存在应用专用 IndexedDB。它们默认不会上传到 GitHub；GitHub 仅用于管理本软件的代码版本。

从 v2.1.0 开始，覆盖安装新版本会继续使用同一个本地资料夹。投递记录格式升级会自动兼容旧数据、保留迁移前快照，并在每次写入前保存最近一次安全副本；简历版本库的升级只会新增结构，不会删除现有简历文件或投递关联。
